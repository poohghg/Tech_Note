...
제시해주신 개념 설명과 질문을 바탕으로 답변드리겠습니다. 핵심은 **읽기만 해도 변경되지 않지만**, **직전에 쓰기 작업이 있었다면** 문제가 발생한다는 것입니다.

---

## 강제 동기 레이아웃(Forced Synchronous Layout)의 원리


### 1. 브라우저의 최적화 (일반적인 동작)

브라우저는 렌더링 성능을 위해 여러 개의 **쓰기(Write)** 작업(DOM 변경, 스타일 변경 등)을 큐에 모아 두었다가, 나중에 한 번의 **레이아웃/리플로우(Layout/Reflow)** 실행으로 처리하려고 합니다. 즉, 쓰기 작업을 **비동기적**으로 배치 처리합니다.
...
문제는 **읽기 API 호출**이 있을 때 발생합니다.

읽기 API(예: `offsetWidth`, `getBoundingClientRect()`, `getComputedStyle()` 등)는 **현재 시점**의 정확한 기하학적 정보(요소의 너비, 위치 등)를 반환해야 합니다.

만약 이 **읽기** 호출이 **직전에 대기 중인 쓰기 작업**이 있는 상태에서 발생하면, 브라우저는 최신 정보를 제공하기 위해 큐에 모아 두었던 쓰기 작업을 모두 실행하고, 그 결과로 **즉시 레이아웃을 강제 실행**합니다. 이것이 바로 **강제 동기 레이아웃**입니다.

### 3. 질문에 대한 답변: 읽기만 하면 변경되지 않아?

> 강제 동기 레이아웃 쓰레싱은 뷰포트내 기하학적 정보를 **읽기만 해도 변경되지 않아**?


- 앞서 수행된 쓰기(Write) 작업과 그 뒤에 오는 읽기(Read) 작업이 섞였을 때**
