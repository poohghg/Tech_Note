...
- 렌더 트리는 DOM트리와 CSSOM 트리를 기반으로 실제로 화면에 표시하는 객체들로 구성된 Render트리를 만든다.
...
- 강제 동기 레이아웃(forced reflow): 최신 스타일/레이아웃이 필요한 읽기 API 호출(offsetWidth, getBoundingClientRect 등)이 앞선 쓰기와 섞이면, 엔진이 즉시 레이아웃을 강제 실행하여 성능 급락(레이아웃 스래싱).
	- 리플로우 과정에서 돔처리는 배치로 이루어지는데, 레이아웃 스래싱이 발생하면 배치가 깨지고 레이아웃이 즉시 실행되어 성능 저하가 발생한다.
##### 리플로우를 유발하는 작업
...
2. DOM 읽기와 쓰기를 분리하여 배치 처리

> 브라우저의 최적화 (일반적인 동작)
> 
> 브라우저는 렌더링 성능을 위해 여러 개의 **쓰기(Write)** 작업(DOM 변경, 스타일 변경 등)을 큐에 모아 두었다가, 나중에 한 번의 **레이아웃/리플로우(Layout/Reflow)** 실행으로 처리하려고 한다. 즉, 쓰기 작업을 **비동기적**으로 배치 처리한다.
