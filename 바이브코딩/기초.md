...
사용자 지정 에어전트는 GitHub Copilot에 특화된 페르소나, 특정 도구 접근 권한 및 도메인 전문 지식을 제공하는 전문 도우미이다. 수동적으로 적용되는 지침이나 개별 작업을 처리하는 스킬과는 달리, 에이전트는 완전한 작업 스타일을 정의한다. 즉, Copilot이 ==생각하는 방식, 사용하는 도구, 그리고 전체 세션 동안 소통하는 방식==을 결정한다.
