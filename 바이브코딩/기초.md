...
> 기술 스택, 빌드 명령어, 코딩 규칙, 금지 사항이 모두 포함된 에이전트는 프로젝트의 전반적인 아키텍처와 도메인 지식을 반영하여, 코파일럿이 일관된 방식으로 코드를 생성하도록 한다. 
> 
> 핵심은 필요한 정보를 적시에 적절하게 제공 전부 쏟아붓는 게 아니라 작업에 필요한 정보만 선별적으로 로딩

#### 에이전트이 크면 실패하는 이유?

1. 컨테스트는 희소 자원: 거대한 지시 파일이 실제 작업, 코드를 밀어냄
2. 모든게 "중요"하면 아무것도 중요하지 않음: 에이전트가 대충 패턴 매칭만 수행
3. 거대한 매뉴얼은 금방 부패: 예전 규칙과 새로운 규칙이 충돌하여 혼란을 초래

#### 중요한 전략

1. 작업에 필요한 정보만 제공: 에이전트가 수행할 특정 작업과 관련된 정보만 포함
2. 에이전트는 특정 작업을 수행하도록 설계: 에이전트가 수행할 작업에 맞게 지침을 구성
3. 에이전트는 특정 도메인 전문 지식을 갖춘 전문가처럼 행동: 에이전트가 특정 도메인에서 전문가처럼 행동하도록 지침을 작성
---

사용자 지정 에어전트는 GitHub Copilot에 특화된 페르소나, 특정 도구 접근 권한 및 도메인 전문 지식을 제공하는 전문 도우미이다. 수동적으로 적용되는 지침이나 개별 작업을 처리하는 스킬과는 달리, 에이전트는 ==완전한 작업 스타일==을 정의한다. 즉, Copilot이 ==생각하는 방식, 사용하는 도구, 그리고 전체 세션 동안 소통하는 방식==을 결정한다.

즉 채팅 창(Copilot Chat)에서 대화할 때, 프로젝트의 전반적인 아키텍처나 주요 기술 스택, 도메인 지식 등을 코파일럿에게 주입하기 위해 사용하는 파일이다. 루트 디렉토리에 `AGENTS.md`를 만들어두면 챗봇이 그 내용을 배경지식으로 깔고 대화한다.
...
- **이름** (권장): 에이전트에 표시할 사람이 읽기 쉬운 이름이다.
- **설명** (필수): 상담원이 하는 일을 명확하게 요약한 내용. 이 내용은 상담원 선택기에 표시되어 사용자가 적합한 상담원을 찾는 데 도움이 된다.
- **모델** (권장): 에이전트를 구동하는 AI 모델. 작업의 복잡성에 따라 선택하세요. 미묘한 추론에는 더 강력한 모델을 사용하는 것이 좋다.
- **도구** (권장): 에이전트가 액세스할 수 있는 내장 도구 및 MCP 서버 모음. 일반적인 도구는 다음과 같다.
...
> 
> Copilot이 특정 작업을 **일관되고 반복 가능한 방식**으로 수행하도록 하기 위해  지침·스크립트·리소스를 묶어둔 **디렉터리 단위 규칙 세트**다.
> 
> 사용자의 프롬프트를 분석해 적절한 스킬을 자동으로 검색하여 호출할 수 있다. 또한 사용자는 슬래시 명령어로 스킬을 직접 호출할 수도 있다. 즉 필요할때 전문 작업을 수행하기 위한 작업 규칙이다.
> 
> 팀단위에서는 **팀의 반복 작업을 규칙화**하여 모든 팀원이 일관되고 높은 품질의 결과를 얻을 수 있도록 지원한다. 예를 들어, 테스트 생성, 코드 리뷰, 문서 작성과 같은 일반적인 워크플로에 대한 스킬을 만들어서 팀이 일관된 방식으로 이러한 작업을 수행하도록 할 수 있다.
...
`SKILL.md`의 구조는 **YAML 프런트매터 + Markdown 본문**으로 구성된다.
...
- 필요에 따라 실행되는 워크플로에는 스킬을 사용하고, 항상 적용되는 표준에는 지침을 사용한다.
	- 코딩 스타일, 네이밍 규칙 -> 사용자 지정 지침
	- 테스트 작정 절차, 장애 분석, 릴리즈 체크리스트 -> 스킬
#### 스킬 vs 에이전트
