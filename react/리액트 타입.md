...
``` 
const node: React.ReactNode = (   <>     Hello     <span>World</span>     {false}     {null}     {["Array", <strong>of</strong>, "nodes"]}   </> );
```
...
`const child1: React.ReactChild = "Hello"; const child2: React.ReactChild = <div>Hi</div>;`
