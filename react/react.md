...
리액트는 이 가상 DOM을 이용해 **실제 DOM 조작 횟수를 최소화하고 효율적으로 업데이트한다.
...
1. 상태(state) or props 변경
2. React가 해당 컴포넌트 다시 렌더 → 새로운 Virtual DOM 트리 생성
3. 이전 Virtual DOM과 비교 (Diffing 알고리즘 사용)
