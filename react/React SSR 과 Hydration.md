...
- `suspense` 는 비동기 작업을 수행하는 구성 요소를 래핑하고, 작업이 진행되는 동안 대체 UI를 표시한 다음, 작업이 완료되면 두성 요소를 교체하는 방식으로 작동한다.
- 스트리밍 서버 렌더링: 서버에서 클라이언트로 HTML 점진적으로 렌더링한다.
- 선택적인 하이드레이션: React는 사용자 상호작용을 기반으로 어떤 구성 요소를 먼저 상호 작용하게 할지 우선순위를 지정한다.
