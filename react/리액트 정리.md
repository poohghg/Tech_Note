...
- JSX 코드가 어떻게 Virtual DOM 객체로 변환되는지 설명해 주세요.
	- JSX 코드는 Babel과 같은 트랜스파일러를 통해 `createElement` 함수 호출로 변환됩니다.
	- `createElement` 함수는 컴포넌트 타입, 속성(props), 자식(children)을 받아 Virtual DOM 객체를 생성합니다.
	- 이 Virtual DOM 객체는 트리 구조로 구성되어, 실제 DOM 구조를 반영합니다.
	- Virtual DOM 객체를 사용하여 UI 상태를 관리하고, 변경 사항을 효율적으로 처리할 수 있습니다.
	- Virtual DOM 객체를 재귀하며 실제 DOM으로 렌더링하는 과정이 필요합니다.
...
- Virtual DOM 은 가장 큰 추상화 중 하나이다.
	- 추상화 대성: 브라우저의 DOM API를 직접 다루지 않고, Virtual DOM을 통해 간접적으로 다룸.
