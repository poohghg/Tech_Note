...
> 자바스크립트 엔진은 자바스크립트 코드를 해석하고 실행하는 역할을 수행한다. 브라우저 런타임에서 자바스크립트는 문서 객체 모델(DOM) 이나 스타일 객체 모델 (CSSOM)을 직접 조작하는데, 이는 실제로 사용자에게 보일 화면에 어떤 요소들이 그려질지를 조작한다는 의미이다. 따라서 자바스크립트 실행하기는 스크립트를 실행한 결과를 바탕으로 렌더링 엔진과 상호 작용하는
