...
- DOM 이란 ? HTML 문서의 계층적인 트리 구조로, 브라우저가 HTML 문서를 이해하고 처리하는 방법을 정의한다. DOM은 HTML 문서의 모든 요소를 객체로 표현하며, 이 객체들은 서로 연결되어 있어 요소들 간의 관계를 표현한다. 브라우저는 이 DOM 트리를 기반으로 화면을 그리게 된다.
