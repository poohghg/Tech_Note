...
``// 브라우저 환경에서 `process.env` 같은 변수 사용 declare const process: {   env: {     NODE_ENV: 'development' | 'production' | 'test';   }; };  console.log(process.env.NODE_ENV); // 타입스크립트는 process.env를 알게 됨``
...
`// 외부 라이브러리가 제공하는 전역 변수 $ 사용 declare const $: (selector: string) => any;  // 사용할 때 타입스크립트 에러 없음 const element = $('div');`
...
`// 전역적으로 사용할 설정 declare const API_URL: string;  fetch(API_URL)   .then(response => response.json())   .then(data => console.log(data));`
...
`// declare const declare const myVar: string; console.log(myVar); // 정상 작동 (myVar는 이미 런타임에 정의되었다고 가정)  // const const myConst = "Hello"; console.log(myConst); // 정상 작동`
