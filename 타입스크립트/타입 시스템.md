...
const p: Person = pw;   // ✅ 가능 (교집합은 부분집합)
```
...
- 대부분의 타입 구조에 해당한다 `string`은 `string | number`의 서브타입이므로, `string` 값을 `string | number` 타입 변수에 할당할 수 있습니다.
	- `Array<string>`은 `Array<string | number>`의 서브타입이다.
- 서브타입 관계가 ==복합 타입==으로 확장될 때, 그 관계가 원래의 서브타입 방향과 동일하게 유지되는 특징이다.
...
let admin: Admin = { name: 'b', isSuperAdmin: true };  
  
user = admin; // Admin은 User의 서브타입이므로 할당 가능
```

### 반공변성(Contravariance)

- 함수 매개변수에 적용된다.
- 예를 들어, `(Admin) => void` 타입의 함수가 필요한 자리에 `(User) => void` 타입의 함수를 전달할 수 있다. `User`가 더 넓은 범위(부모)이고 `Admin`이 더 좁은 범위(자식)일 때, 더 넓은 타입을 다룰 수 있는 함수가 더 좁은 곳에서도 안전하게 동작하기 때문입니다.
- `반대 방향으로 변하는` 특성을 가진다.
