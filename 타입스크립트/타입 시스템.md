...
- `A | B` 는 “A 또는 B 중 하나”를 의미합니다.
- 즉, **값의 집합 관점**으로 보면 `A | B`는 `A`와 `B`를 모두 포함한 **더 큰 집합**입니다.  
    → 따라서 `A`나 `B`는 `A | B`의 **부분집합** → 즉, 서브타입입니다.
    

`type A = "a"; type B = "b"; type AB = A | B;  let x: AB;  const a: A = "a"; x = a;  // ✅ 가능 (A는 AB의 부분집합)`
