...
처음에 실행되는 코드는 Global Execution Context(GEC)에서 실행된다. 처음에 콜스택에 담기는 것은 GEC이고, 여기에서 호출하는 다른 함수는 콜 스택의 맨 위에 running exection context로 추가되어 실행된다. 그 함수의 실행이 끝나면 해당 함수의 실행 컨텍스트는 콜 스택에서 제거되고, 모든 코드가 실행되면 GEC까지 제거되어 실행이 종료된다.
