...
4. 스크립트를 실행할 준비를 하기 위한 단계는 다음과 같다.
5. 설정의 realm 실행 컨텍스트를 JavaScript 실행 컨텍스트 스택에 푸시한다. 이제 실행중인 JavaScript 실행 컨텍스트가 된다.
6. 환경 설정 객체 settings로 콜백을 실행할 준비를 하기 위한 단계는 다음과 같다.
7. 설정을 백업 임원 설정 객체 스택에 푸시한다.
8. context를 최상위 스크립트 실행 컨텍스트로 한다.
9. 만약 스크립트 실행 컨텍스트가 null이 아니라면, 스크립트 실행 컨텍스트를 JavaScript 실행 컨텍스트 스택에 푸시한다.
