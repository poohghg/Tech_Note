...
> 코드베이스의 특정 파일이나 디렉터리를 사용할 때 GitHub Copilot의 동작을 자동으로 안내하는 영구 구성 파일이다. 사용자가 직접 호출하거나 에이전트가 실행해야 하는 스킬과 달리, 사용자 지정 지침은 백그라운드에서 조용히 작동하여 Copilot이 팀의 표준, 규칙 및 아키텍처 결정을 일관되게 준수하도록 합니다.
