...
> 또한 팀단위에서는 **팀의 반복 작업을 “규칙화**하여 모든 팀원이 일관되고 높은 품질의 결과를 얻을 수 있도록 지원한다.** 예를 들어, 테스트 생성, 코드 리뷰, 문서 작성과 같은 일반적인 워크플로에 대한 스킬을 만들어서 팀이 일관된 방식으로 이러한 작업을 수행하도록 할 수 있다.
...
`SKILL.md`는 **YAML 프런트매터 + Markdown 본문**으로 구성된다.
