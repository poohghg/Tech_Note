...
- O(1)상수시간 :n의 값이 커져도 실행시간은 변하지 않는다.
    ...
    - ![](http://localhost:63342/markdownPreview/321051850/src/prev/1.big_o?_ijt=j09ookqa0uico8pu8fi6p6ke1r)
        
        `function o1(n) {   // n의 크기와 상관업이 3번의 연산만 진행된다.   // -> 3   return (n * (n + 1)) / 2; }`
        
- O(logN)로그시간:n의값이 커질수록 실행시간이 logN에 비례해서 증가한다.
    
    - ![](http://localhost:63342/markdownPreview/321051850/src/prev/1.big_o?_ijt=j09ookqa0uico8pu8fi6p6ke1r)
        
        `function logN(n) {   // i값이 반복할때마다 2배씩 증가한다.   // log8 = 3   let sum = 0;   for (let i = 0; i <= n; i * 2) {     const element = array[i];   } }`
...
- O(n)선형시간 :n이 커질수록 실행시간은 선형적으로 늘어난다.
    ...
    - ![](http://localhost:63342/markdownPreview/321051850/src/prev/1.big_o?_ijt=j09ookqa0uico8pu8fi6p6ke1r)
        ...
        `function on(n) {   // 실행시간은 n의 크기만큼 늘어난다.   // 2n + 1 -> 실제 2n이든 5n이든 전체적인 큰시간차이는 없다.   let answer = 0;   for (let i = 1; i <= n; i++) {     answer += i;   }   for (let i = 1; i <= n; i++) {     answer += i;   }   return answer; }`
