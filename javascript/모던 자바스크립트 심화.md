...
> -  require()는 동기(sync)적으로 모듈을 불러오고, ESM 모듈 로딩은 비동기(async)적으로 이루어진다.
> - 
...
- 따라서 ESM에서 CJS를 `import` 할 수는 있지만, CJS에서 ESM을 `require` 할 수는 없습니다. 왜냐하면 CJS는 Top-level Await을 지원하지 않기 때문이다.
- 이 외에도 두 Module System은 기본적으로 동작이 다르다.
- 따라서 두 Module System은 서로 호환되기 어렵다.

#### 모듈 시스템 확인

##### package.json을 통해 모듈 시스템 확인

- `.js` 파일의 Module System은 package.json의 `type` field에 따라 결정된다.
    - `type` field의 기본값은 `"commonjs"` 이고, 이 때 `.js` 는 CJS로 해석됩니다.
    - 다른 하나는 `"module"` 이다. 이 때 `.js` 는 ESM으로 해석된다.
- .`.cjs` 는 항상 CJS로 해석되고, `.mjs` 는 항상 ESM으로 해석된다.```

##### tscconfig.json을 통해 모듈 시스템 확인 (4.7 이상)

> `tsconfig.json` 의 `moduleResolution` 이 `nodenext` 또는 `node16` 으로 설정된 경우, 위 규칙이 똑같이 적용된다.
...
- 하지만 CJS는 트리쉐이킹 어렵다는 단점이 있다.
	...
	- 동적로드란? 런타임에 모듈을 로드하는 방식으로, require() 함수에 변수나 표현식을 사용할 수 있다.
- 브라우저 환경에서 사용하기 어렵다
...
// module.exports 
function foo() { if (/* 동적인 조건 */) { module.exports = /* ... */; } } foo();


```

- CJS는 기본적으로 `require` / `module.exports` 를 동적으로 하는 것에 아무런 제약이 없다.
- 따라서 CJS 모듈은 런타임에 모듈의 구조가 결정되므로, 정적 분석이 어렵고 트리 쉐이킹이 불가능하다.
- 즉 빌드 타임에 정적 분석을 적용하기 어렵고, 런타임에서만 모듈 관계를 파악할 수 있다.
#### ES Module의 특징
...
function foo() { export const value = "foo"; // 불가능 } 
...
2. 이 상태 변경이 감지되면, **`.then()` 등에 등록된 콜백 함수**가 **Microtask Queue** 에 예약된다.
3. **Microtask Queue**에 있는 작업들은 **Call Stack의 동기 작업이 모두 끝난 직후**, **다음 이벤트 루프 틱이 시작되기 전에** 실행된다. (일반적인 `setTimeout(..., 0)`이나 I/O 작업 콜백보다 **우선순위가 높다**.)
...
`async/await` 함수에서 `await` 키워드를 만나면 다음과 같은 일이 발생한다.
