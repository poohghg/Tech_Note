...
프로미스의 **진정한 비동기적 부분**은 **`.then()`**, **`.catch()`**, **`.finally()`** 등에 등록된 콜백 함수들입니다.
1. 실행자 안에서 `resolve()`나 `reject()`가 호출되면 프로미스의 상태가 변경됩니다.
2. 이 상태 변경이 감지되면, **`.then()` 등에 등록된 콜백 함수**가 **Microtask Queue**라는 특별한 비동기 대기열에 들어갑니다.
3. **Microtask Queue**에 있는 작업들은 **Call Stack의 동기 작업이 모두 끝난 직후**, **다음 이벤트 루프 틱이 시작되기 전에** 실행됩니다. (일반적인 `setTimeout(..., 0)`이나 I/O 작업 콜백보다 **우선순위가 높습니다**.)
