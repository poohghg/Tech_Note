...
- `let x = 10 + 20;`에서 `10 + 20`은 표현식이고 `let x = ...;`은 표현문이다.
- 메모리 관점에서
...
반복자는 두 개의 속성( `value`, `done`)을 반환하는 next() 메소드 사용하여 객체의 [Iterator protocol](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Iteration_protocols#the_iterator_protocol)을 구현한다. 시퀀스의 마지막 값이 산출되었다면 `done`값은 `ture`가 된다.
...
- 이터러블 프로토콜이란
	- 오브젝트가 반복할 수 있는 구조이어여 한다.
	- `Symbol.iterator`속성을 가지고 있어야 한다.
	-  [언어와 순회 프로토콜 간의 상호 작용](https://developer.mozilla.org/ko/docs/Web/JavaScript/Reference/Iteration_protocols#%EC%96%B8%EC%96%B4%EC%99%80_%EC%88%9C%ED%9A%8C_%ED%94%84%EB%A1%9C%ED%86%A0%EC%BD%9C_%EA%B0%84%EC%9D%98_%EC%83%81%ED%98%B8_%EC%9E%91%EC%9A%A9) 참조
		- `prototype` 객체가 `@@iterator` 메서드를 구현하기 때문에 모두 내장 순회 가능하다.
- 이터러블 오브젝트
	- 이터러블 프로토콜을 갖고 있는 오브젝트이다.
	- 반복 구조, `Symbol.iterator` 프로토타입 프로퍼티를 가지고 있다.
		- 스펙에서는 `@@iterator` 표기
...
- 순회 가능 프로토콜을 사용하면 자바스크립트 객체를 `for..of` 구조에서 반복되는 값과 같은 순회 동작을 정의하거나 사용자 정의가 가능하다.
- 오브젝트와 달리 반복 가능해야 한다.
