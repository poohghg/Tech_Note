...
- **표현식**은 항상 값을 생성하며, 엔진은 이를 평가하고 필요한 경우 메모리에 저장합니다.
- **표현문**은 특정 작업을 수행하며, 값 반환보다 프로그램의 흐름 제어에 초점이 맞춰져 있습니다.
- 자바스크립트 엔진은 이 둘을 처리하기 위해 **실행 컨텍스트와 스코프 체인, 메모리 관리**를 적절히 활용합니다.
