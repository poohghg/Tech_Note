...
- EC(Execution Contexts): 실행 컨테스트
	...
	- VEC(Variable Environment Component): 변수 환경 컴포넌트: 컨테스트 내부의 식별자 정
	- TBC: this 바인딩 컴포넌트
...
- function 키워드를 만나면
  ...
  - 함수 키워드를 만났을때 함수밖(상위)의 스코프가 결정된다.
	  - 이는 정적으로 결정된다.
- 함수가 호출되면
  - function 오브젝트의 [[Scope]]를 실행 컨테스트의 렉시컬 환경 컴포넌트의 외 외부 렉시컬 환경 참조에 설정
- 고전적인 var의 문제
