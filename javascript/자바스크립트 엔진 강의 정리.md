...
> **① 표준 빌트인 객체**
> 
> - 자바스크립트에서 기본적으로 제공하는 객체 중 ECMAScript 사양에 정의된 표준 객체. 애플리케이션 전역의 공통 기능을 제공합니다
...
> **② 호스트 객체**
> 
> - ECMAScript 사양에 정의되어 있지 않지만 ==자바스크립트 실행 환경(브라우저 또는 Node.js 환경)에서 추가로 제공하는 객체==를 말한다
...
==함수는 자바스크립트에서 클로저 역할==을 하기 때문에 스코프를 생성한다. ==함수 내에 정의된 변수는 외부 함수나 다름 함수 내에서 접근 할 수 없다.==
...
- 실행 컨테스트 생성 시 식별자 해결을 위해 사용한다.
	...
	- var, function 오브젝트는 호이스이팅 일어난다.
		...
		- function 선언식은 function 오브젝트로 할당.
			- 함수 선언문의 경우 선언,초기화,할당 단계가 내부적으로 동시에 일어난다.
...
// "호이스팅"
// 초기화 단계에서 값이 있으면 할당하지 않음으로 인해 book 함수 오브젝트가 할당되어 있기 때문에 book 함수 오브젝트가 호출되고 결과값이 출력된다.
...
- 실행중인 function 오브젝트에 작성한 ==변수,함수를 선언적 환경 레코드==에 설정
