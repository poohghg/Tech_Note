...
2. base 변수에 값을 할당
3. 준비 단계
4. getPoint 함수 호출 -> getPoint 함수로 이동 
	1. 엔진은 실행 컨텍스트를 생성하고 실행 컨텍스트 안으로 이동
5. 실행 컨텍스트의 컴포넌트를 생성하여 실행 컨텍스트에 첨부
	1. 렉시컬 환경 컴포넌트
	2. 변수 환경 컴포넌트
	3. this 바인딩 컴포넌트
6. 렉시컬 환경 컴포넌트의 환경 레코드를 생성
	1. 함수 안의 함수, 변수를 바인딩
7. 외부 렉시컬 환경 참조를 생성하여 렉시컬 환경 컴포넌트에 첨부
	1.  함수 객체 내부 프로퍼티인 __ scope __ 값을 참조
8. 호출한 함수의 파라미터 값을
	1. 호출된 함수의 파라미터 이름에 매핑
	2. 환경 레코드에 작성
9. 함수 선언문을 function 오브젝트로 생성 한다.
	1. 함수 {name: function 오브젝트} 형태
10. 함수 표현식과 변수에 초기값을 설정한다.
	1. 초기값은 undefined 이다
