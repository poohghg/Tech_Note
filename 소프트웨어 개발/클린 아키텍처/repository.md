...
- 교체 용이성: API를 변경해도 인터페이스를 구현하는 새로운 클래스만 작성하면 되므로, 도메인 계층(컴포넌트)에는 영향을 미치지 않는다.
- 테스트 용이성: 컴포넌트를 테스트할 때, 실제 API 호출 대신 인터페이스를 구현한 Mock 객체를 주입하여 테스트할 수 있다.
- 명확한 역할 분리: 컴포넌트는 데이터를 어떻게 가져오는지 신경 쓰지 않고, 오직 비지니스 로직에만 집중할 수 있다.