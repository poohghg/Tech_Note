...
2. widgets: 크고 독립적으로 동작하는 UI 구성 단위이다. 일반적으로 하나의 완결된 화면 기능(use case)을 제공한다.
...
2. features: 사용자에게 비지니스 가치를 제공하는 액션을 구현한 재사용 가능한 제품 기능 단위이다.
	- 특정 도메인의 기능을 담당한다.
	- 기능 중심이 아니라 도메인 중심 으로 설계 한다.
   
5. entities: 이 레이어는 비즈니스 엔티티를 나타
