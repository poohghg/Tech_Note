...
1. processes: 이 레이어는 여러 단계로 이루어진 등록과 같이 여러 페이지에 걸쳐 있는 프로세스를 처리합니다. 이 레이어는 더 이상 사용되지 않는다. 선택적 레이어이다.
   
2. pages: 이 레이어에는 애플리케이션의 페이지가 포함된다.
...
1. widgets: 페이지에 사용되는 독립적인 UI 컴포넌트이다.
...
1. features: 이 레이어는 비즈니스 가치를 전달하는 사용자 시나리오와 기능을 정의한다.. 예를 들어 좋아요, 리뷰 작성, 제품 평가 등이 있습니다. 선택적 레이어이다.
   - 특정 도메인의 기능을 담당한다.
   
1. entities: 이 레이어는 비즈니스 엔티티를 나타냅니다. 선택적 레이어다. 
